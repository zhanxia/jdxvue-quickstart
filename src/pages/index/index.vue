<template>
  <div>
    ...
    <div v-for="item in indexData.bannerList" class="banner-item" :key="index">
...
export default {
  ...
  methods: {
    ...
    _goPage(url){
      if (jddvuePlatform != 'h5') {
        url = `../webview/index?pageUrl=${encodeURIComponent(url)}`;
        jddvue.navigateTo({
          url
        })
      } else {
    ...
    _goNativePage(){
      ...
      if (jddvuePlatform != 'h5') {
        const url = `../webview/index?pageUrl=${encodeURIComponent(h5url)}`;
        jddvue.navigateTo({
          url
        })
      } else {
